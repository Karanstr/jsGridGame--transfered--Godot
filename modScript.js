...
window.Non = function (degree) {
  ...
  for (let j = 0; j < degree; j++) {
    let b = 0;
    for (let i = 0; i < 2 ** (j * 2); i++) { b += i.toString(2).length }
    //console.log(b + ' ' + j)
    totalStorage += b;
  }
  return totalStorage
...
window.With = function (degree) {
  let totalStorage = 0;
  for (let j = 0; j < degree; j++) {
    let b = 0;
    for (let i = 0; i < 2 ** (j * 2); i++) {
      let newVal = i + 2 ** (j * 2);
      b += newVal.toString(2).length;
    }
    //console.log(b + ' ' + j)
    totalStorage += b;
  }
...
window.p = new Region(145, 185, 20, 20, new blockMap('#ffffff'));
